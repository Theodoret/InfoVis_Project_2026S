...
function linearTrendForPoints(points, xKey = 'gdp', yKey = 'value') {
  const pairs = (points || [])
  ...
  pairs.forEach(([xValue, yValue]) => {
    const xDifference = xValue - xMean;
    numerator += xDifference * (yValue - yMean);
...
function clippedTrendPoints(points, xKey, yKey, xDomain, yDomain) {
  const trend = linearTrendForPoints(points, xKey, yKey);
  if (!trend) {
    return [];
  }
  ...
  if (trend.slope !== 0) {
    candidates.push((yMin - trend.intercept) / trend.slope);
    candidates.push((yMax - trend.intercept) / trend.slope);
  }

  const epsilon = 1e-9;
  const visibleXValues = [...new Set(
    candidates
      .filter(value => Number.isFinite(value) && value >= xMin - epsilon && value <= xMax + epsilon)
      .map(value => Math.max(xMin, Math.min(xMax, value)))
      .map(value => Number(value.toPrecision(12)))
  )]
    .map(xValue => ({
      [xKey]: xValue,
      [yKey]: trend.slope * xValue + trend.intercept,
    }))
    .filter(point => point[yKey] >= yMin - epsilon && point[yKey] <= yMax + epsilon)
    .map(point => ({
      [xKey]: point[xKey],
      [yKey]: Math.max(yMin, Math.min(yMax, point[yKey])),
    }))
    .sort((left, right) => left[xKey] - right[xKey]);

  if (visibleXValues.length >= 2) {
    return [visibleXValues[0], visibleXValues[visibleXValues.length - 1]];
  }
...
function renderScatterPlot(options = {}) {
  ...
  let yExtent = d3.extent(points, point => point[yKey]);
  if (Array.isArray(options.yDomain) && options.yDomain.length === 2) {
  ...
  const plotClipId = `scatter-plot-${Math.round(Math.random() * 1000000)}`;
  ...
  const trendPoints = options.showTrend === false
    ? []
    : clippedTrendPoints(points, xKey, yKey, x.domain(), y.domain());
